...
export default function CheckPage() {
  ...
  const scheduleGroups = useMemo(() => {
    ...
    // 🔹 ใหม่: กำหนดการ "ฉีดพ่น" ต้องมีขั้นต่ำ 4 ครั้ง
    // ครั้งที่ 1 = วันเริ่มสัญญา, แต่ละครั้งห่างกัน 3 เดือน
    if (pkgKey === "spray" && start) {
      ...
      if (!sprayDates || sprayDates.length < 4) {
        // ถ้าไม่มีหรือมีไม่ครบ 4 ครั้ง → ใช้ตารางคำนวณใหม่ทั้งหมด
        sprayDates = autoSpray;
      } else {
        // ถ้ามี ≥ 4 แล้ว → บังคับให้ "ครั้งที่ 1" ตรงกับวันเริ่มสัญญา
